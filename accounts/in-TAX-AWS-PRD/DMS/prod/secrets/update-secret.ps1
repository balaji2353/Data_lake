...
if (-not $SecretName -or -not $SecretString) {
  Write-Error "Usage: ./update-secret.ps1 -SecretName <name> -SecretString '<json-string>'"
  exit 1
}

aws secretsmanager update-secret --secret-id $SecretName --secret-string $SecretString
Param(
